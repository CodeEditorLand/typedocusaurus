/******/ (() => {
  ...
  paths: Array<string>
			...
			exports.default = (
				...
				_module2.default._load = function (
					request /*: string*/,
					parent /*: ParentType*/,
				) {
		...
		/***/ 879: /***/ (
			...
			const external_typedoc_namespaceObject = require("typedoc"); // CONCATENATED MODULE: external "typedoc-plugin-markdown"
			...
			var reflection_title = __nccwpck_require__(794); // CONCATENATED MODULE: ./src/front-matter.ts
			var __decorate =
...
${Object.entries(vars)
	.map(
		([key, value]) =>
			`${key}: ${typeof value === "string" ? `"${escapeString(value)}"` : value}`,
	)
	.join("\n")}
---`;
				return yaml;
			...
			let FrontMatterComponent = class FrontMatterComponent extends output_components.RendererComponent {
				...
				getId(page) {
					return external_path_.basename(
						page.url,
						external_path_.extname(page.url),
					);
				}
				getTitle(page) {
			...
			__decorate(
				...
				FrontMatterComponent.prototype,
				"out",
				void 0,
			);
			__decorate(
			...
			__decorate(
				[
					(0, external_typedoc_namespaceObject.BindOption)(
						"globalsTitle",
					),
				],
				FrontMatterComponent.prototype,
			...
			const getOptions = (siteDir, opts) => {
				...
				if (opts.sidebar === null) {
					...
					options = {
						...
						sidebar: {
							...sidebar,
							sidebarPath: external_path_.resolve(
								siteDir,
								sidebar.sidebarFile,
							),
						},
				...
				options = {
					...options,
					siteDir,
					outputDirectory: external_path_.resolve(
						siteDir,
						options.docsRoot,
						options.out,
					),
			...
			const addOptions = (app) => {
				...
				app.options.addReader(
					new external_typedoc_namespaceObject.TypeDocReader(),
				);
				app.options.addReader(
					new external_typedoc_namespaceObject.TSConfigReader(),
				);
			...
			async function render(project, outputDirectory) {
				...
				const output = new events.RendererEvent(
					events.RendererEvent.BEGIN,
					outputDirectory,
					project,
				);
				...
				if (!output.isDefaultPrevented) {
					(_a = output.urls) === null || _a === void 0
						? void 0
						: _a.forEach((mapping, i) => {
								var _a;
								this.renderDocument(
									output.createPageEvent(mapping),
								);
								console.log(
									`\rGenerated ${i + 1} of ${(_a = output.urls) === null || _a === void 0 ? void 0 : _a.length} TypeDoc docs`,
								);
							});
					console.log(`\n`);
			...
			var external_fs_ = __nccwpck_require__(747); // CONCATENATED MODULE: ./src/sidebar.ts
			var sidebar_decorate =
				(undefined && undefined.__decorate) ||
				function (decorators, target, key, desc) {
					var c = arguments.length,
						r =
			...
			let SidebarComponent = class SidebarComponent extends output_components.RendererComponent {
				...
				async onRendererBegin(renderer) {
					...
					// @ts-ignore
					const navigation =
						(_a = this.application.renderer.theme) === null ||
						_a === void 0
							? void 0
							: _a.getNavigation(renderer.project);
	...
	(() => {
		...
		(async () => {
			try {
				...
				const overrideCondition = (request) =>
					request.startsWith("typedoc");
				const resolveRequest = (request) =>
					require(
						path.normalize(
							process.cwd().replace("/dist/typedocusaurus", "") +
								`/${originPath}node_modules/${request}`,
						),
					);
